...
public class ImageTool {
	 ...
	 * @return
	 */
	public ImageTool midddleValueFilter(int whiteAreaMinPercent,
			boolean removeLighter) {
		int modify = 0;
		int avg = getAvgValue();
		Color point;
		while (getWhitePercent() < whiteAreaMinPercent) {
			for (int i = 0; i < height; i++) {
				for (int j = 0; j < width; j++) {
					point = new Color(image.getRGB(j, i));
					if (removeLighter) {
						if (((point.getRed() + point.getGreen() + point
								.getBlue()) / 3) > avg - modify) {
							// System.out.println(((point.getRed() +
							// point.getGreen() + point.getBlue()) /
							// 3)+"--"+(avg - modify));
	 ...
	 * @return
	 */
	public ImageTool removeBadBlock(int blockWidth, int blockHeight,
			int neighborhoodMinCount) {
		int val;
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				if (isBlackBlock(x, y, blockWidth, blockHeight)) {// 只有当块是全黑色才计算
					for (int x1 = topLeftXIndex; x1 <= topLeftXIndex
							+ blockWidth + 1; x1++) {
						for (int y1 = topLeftYIndex; y1 <= topLeftYIndex
								+ blockHeight + 1; y1++) {
							// 判断这个点是否存在
							if (x1 < width && x1 >= 0 && y1 < height && y1 >= 0) {
								// 判断这个点是否是传入矩形的外围点
								if (x1 == topLeftXIndex
										|| x1 == topLeftXIndex + blockWidth + 1
										|| y1 == topLeftYIndex
										|| y1 == topLeftYIndex + blockHeight
												+ 1) {
									// 这里假定图像已经被黑白化,取Red值认为不是0就是255
									val = new Color(image.getRGB(x1, y1))
											.getRed();
	...
	private boolean isBlackBlock(int startX, int startY, int blockWidth,
			int blockHeight) {
		...
		// System.out.println(startX + "--" + startY + "" + (counter ==
		// total&&total!=0));
